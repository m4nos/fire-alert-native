...
export const fetchSlots = createAsyncThunk('slots/fetchSlots', async ({ shiftId }: { shiftId: Shift['id'] }) => {
  try {
    const slotsCollection = query(collection(FirebaseStore, 'shifts', shiftId, 'slots'))
...
export const reserveSlot = createAsyncThunk(
  'slots/reserveSlot',
  async ({ shiftId, slotId }: { shiftId: Shift['id']; slotId: Slot['id'] }, { getState }) => {
    const {
      userSlice: { appUser }
